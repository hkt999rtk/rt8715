...
static int carplay_pro1_get_fw_layout(carplay_pro1_ota_state_t *state)
{
	...
	if (state->curr_fw_addr == UINT32_MAX || state->new_fw_addr == UINT32_MAX ||
		...
		printf("[carplay_ota] invalid fw addr curr=0x%08x new=0x%08x\n",
				state->curr_fw_addr, state->new_fw_addr);
		return CARPLAY_OTA_RET_ERR;
	}

	state->target_fw_size = CARPLAY_OTA_DEFAULT_FW_SLOT_SIZE;
	printf("[carplay_ota] curr=0x%08x target=0x%08x size=0x%08x\n",
			state->curr_fw_addr, state->new_fw_addr, state->target_fw_size);
...
static int carplay_ota_erase_to(carplay_pro1_ota_state_t *state, uint32_t end_offset)
{
	if (end_offset > state->target_fw_size) {
		printf("[carplay_ota] erase range overflow end=0x%08x size=0x%08x\n",
				end_offset, state->target_fw_size);
		return CARPLAY_OTA_RET_ERR;
	}
								   ...
								   const uint8_t *buf, uint32_t len)
{
	...
	if (offset > state->target_fw_size || len > (state->target_fw_size - offset)) {
		printf("[carplay_ota] write overflow offset=0x%08x len=0x%08x size=0x%08x\n",
				offset, len, state->target_fw_size);
		return CARPLAY_OTA_RET_ERR;
	}
								...
								const uint8_t *buf, uint32_t len)
{
	...
	while (checked < len) {
		...
		if (carplay_flash_read(state, offset + checked, verify_buf, chunk) != CARPLAY_OTA_RET_OK) {
			printf("[carplay_ota] readback failed offset=0x%08x len=0x%08x\n",
					offset + checked, chunk);
			return CARPLAY_OTA_RET_ERR;
		}

		if (memcmp(verify_buf, buf + checked, chunk) != 0) {
			printf("[carplay_ota] verify mismatch offset=0x%08x len=0x%08x\n",
					offset + checked, chunk);
			return CARPLAY_OTA_RET_ERR;
		}
									  ...
									  const uint8_t *buf, uint32_t len)
{
	...
	if (carplay_flash_write_raw(state, offset, buf, len) != CARPLAY_OTA_RET_OK) {
		printf("[carplay_ota] flash write failed offset=0x%08x len=0x%08x\n", offset, len);
		return CARPLAY_OTA_RET_ERR;
	}
...
static int carplay_parse_smart_header(carplay_pro1_ota_state_t *state)
{
	...
	carplay_switch_to_image(state, 0U);
...
static int carplay_fatfs_erase_to(carplay_pro1_ota_state_t *state, uint32_t end_offset)
{
	if (end_offset > state->fatfs_size) {
		printf("[carplay_ota] fatfs erase overflow end=0x%08x size=0x%08x\n",
		       end_offset, state->fatfs_size);
		return CARPLAY_OTA_RET_ERR;
	}

	while (state->erased_until < end_offset) {
		device_mutex_lock(RT_DEV_LOCK_FLASH);
		flash_erase_sector(&state->flash, state->fatfs_base + state->erased_until);
		device_mutex_unlock(RT_DEV_LOCK_FLASH);
		state->erased_until += CARPLAY_OTA_SECTOR_SIZE;
	}
				      ...
				      uint32_t offset, const uint8_t *buf, uint32_t len)
{
	...
	device_mutex_unlock(RT_DEV_LOCK_FLASH);
	if (ret != 1) {
		printf("[carplay_ota] fatfs write failed offset=0x%08x len=0x%08x\n",
		       offset, len);
		return CARPLAY_OTA_RET_ERR;
	}
	...
	while (checked < len) {
		...
		if (ret != 1 || memcmp(verify_buf, buf + checked, chunk) != 0) {
			printf("[carplay_ota] fatfs verify fail offset=0x%08x\n",
			       offset + checked);
			return CARPLAY_OTA_RET_ERR;
		}
		checked += chunk;
...
static int carplay_ota_prepare_after_signature(carplay_pro1_ota_state_t *state)
{
	...
	if (carplay_flash_read_abs(state, state->curr_fw_addr, current_sig, sizeof(current_sig)) !=
		CARPLAY_OTA_RET_OK) {
		printf("[carplay_ota] read current fw signature failed addr=0x%08x\n", state->curr_fw_addr);
		return CARPLAY_OTA_RET_ERR;
	}
...
static int carplay_ota_finalize(carplay_pro1_ota_state_t *state)
{
	...
	/* If last image was FATFS, no signature needed */
	if (state->is_fatfs_ota != 0U) {
		state->finalized = 1U;
		carplay_flash_percent = 100;
		printf("[carplay_ota] fatfs finalize done, written=0x%08x\n",
		       state->fatfs_write_offset);
	...
	if (state->signature_len != CARPLAY_OTA_SIGNATURE_LEN ||
		...
		printf("[carplay_ota] incomplete image sig_len=%u placeholder=%u write=0x%08x\n",
				state->signature_len, state->placeholder_written, state->write_offset);
		return CARPLAY_OTA_RET_ERR;
	}
	...
	state->finalized = 1U;
	carplay_flash_percent = 100;
	printf("[carplay_ota] finalize done, written=0x%08x\n", state->write_offset);
	return CARPLAY_OTA_RET_FINISH;
}
...
int ota_update_init(carplay_ota_context_t *ctx, char *host, int port, char *resource, uint8_t type)
{
	...
	printf("[carplay_ota] ota_update_init type=%u; HTTP POST OTA path uses "
			"deinit-time finalize on Pro1", type);
...
int ota_update_deinit(carplay_ota_context_t *ctx)
{
	...
	if (state != NULL) {
		if (state->failed == 0U && state->finalized == 0U) {
			if (state->is_fatfs_ota != 0U) {
				if (state->fatfs_write_offset > 0U) {
					printf("[carplay_ota] ota_update_deinit fatfs auto-finalize, "
					       "written=0x%08x\n", state->fatfs_write_offset);
			...
			} else if (state->write_offset > CARPLAY_OTA_SIGNATURE_LEN) {
				printf("[carplay_ota] ota_update_deinit auto-finalize, written=0x%08x\n",
						state->write_offset);
				finalize_ret = carplay_ota_finalize(state);
				if (finalize_ret == CARPLAY_OTA_RET_ERR) {
					state->failed = 1U;
					printf("[carplay_ota] ota_update_deinit auto-finalize failed\n");
				}
			}
		} else {
			printf("[carplay_ota] ota_update_deinit skip finalize failed=%u finalized=%u "
					"written=0x%08x\n",
					state->failed, state->finalized, state->write_offset);
		}
...
int download_fw_program(carplay_ota_context_t *ctx, const uint8_t *buf, uint32_t len)
{
	...
	while (input_offset < len) {
		...
		/* If we've reached or passed the current image boundary, switch */
		if (state->received_len >= boundary &&
		    state->current_img_idx + 1U < state->smart_hdr_num) {
			if (carplay_switch_to_image(state, (uint8_t)(state->current_img_idx + 1U))
			    != CARPLAY_OTA_RET_OK) {
				state->failed = 1U;
				return CARPLAY_OTA_RET_ERR;
			}
		...
		if (chunk == 0U) {
			/* Should not happen unless received_len == boundary */
			state->received_len++;
			input_offset++;
			continue;
		}
	...
	/* Buffered POST complete? */
	if (state->buffered_post_mode != 0U && state->expected_input_len > 0U &&
	    state->received_len >= state->expected_input_len &&
	    state->finalized == 0U) {
		printf("[carplay_ota] buffered POST complete, received=%lu expected=%lu; finalize\n",
