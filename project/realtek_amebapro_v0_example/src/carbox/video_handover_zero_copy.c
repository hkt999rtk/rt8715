...
void *carbox_video_handover_destination_malloc(size_t length)
{
	...
	if ((active != NULL) && !active->substituted &&
	...
	/* All validation occurs before returning the source pointer.  Once the
	 * substitution is committed there is intentionally no partial fallback:
	 * the two object-local free hooks own the lifetime transaction. */
	result = (result != NULL) ? result : malloc(length);
					   ...
					   const void *source, size_t length)
{
	...
	if ((active != NULL) && active->substituted &&
		...
		video_handover_stats.bytes_saved += length;
		elide = 1;
	}
	taskEXIT_CRITICAL();
	return elide;
...
void carbox_video_handover_end(void)
{
	...
	active = video_handover_find_active_locked(task);
	if (active != NULL) {
		if (active->substituted && !active->memcpy_elided) {
	...
	taskEXIT_CRITICAL();
	if (bad_pointer != NULL) {
		video_handover_disable("missing-alias-memcpy", bad_pointer);
	}
}
...
void carbox_video_handover_report(uint32_t sequence)
{
	...
	rt_printf("[VIDEOHOF][%lu] source_alloc/table_full/active_full="
		  "%lu/%lu/%lu release producer/consumer/final="
