...
#include "media_doorbell_broadcast.h"
#include "sockets.h"
...
void doorbell_broadcast_set_packet_callback(void (*callback)(uint8_t *packet, size_t len))
{
...
static void doorbell_broadcast_thread(void *param)
{
...
void media_doorbell_broadcast(void)
{
	/*user can start their own task here*/
	if(xTaskCreate(doorbell_broadcast_thread, ((const char*)"doorbell_broadcast"), 256, NULL, tskIDLE_PRIORITY + 1, NULL) != pdPASS) {
		printf("\r\n media_doorbell_broadcast: Create Task Error\n");
	}
}
