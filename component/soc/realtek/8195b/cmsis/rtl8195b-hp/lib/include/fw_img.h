...
#define MAX_PARTITION_RECORD        (17)
...
#define PARTITION_TBL_USER_DATA_SIZE       (3456)

#define FLASH_CAL_PATTER_SIZE       32
#if CHIP_VER == CHIP_A_CUT
#define OTA_SIGNATURE_SIZE          16
#else
...
enum FW_PImageType_e
{
    ...
    FW_IMG_WOWLN           = 0x0A, // FW image for wake on WLan
    FW_IMG_CINIT           = 0x0B, // FW image for System Customer initialization during booting
...
typedef struct fw_part_info_s {
    uint32_t magic_num;   /*!< The pattern to validate the partition table data */  
...
#if CHIP_VER == CHIP_A_CUT
    uint8_t sign[3];            /*!< offset 0x15: the pattern to validate the decrypted image, the signature is "RTK" */
    uint8_t reserve2[8];        /*!< offset 0x18: reserved */
#else
...
typedef struct hal_flash_boot_stubs_s {
    ...
    void (*erase_boot_loader)(uint32_t code_start, uint32_t code_size, uint32_t img2_entry, 
                              hal_xip_sce_cfg_t *pxip_sce_cfg, hal_ddr_sce_cfg_t *pddr_sce_cfg);
