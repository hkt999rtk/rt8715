...
void uart_irq(u32 id,u32 event)
{
	...
	/*
	 * This callback runs in UART_IRQn context.  Do not echo with
	 * stdio_port_putc()/uart_send_string() here: the stdio TX path can enter
	 * a normal FreeRTOS critical section when it is busy, which is illegal
	 * from an ISR.  The command itself is still delivered to log_service by
	 * the FromISR semaphore below; command output is printed by task context.
	 */
	if(event == RxIrq) {
		//rc = serial_getc(sobj);
