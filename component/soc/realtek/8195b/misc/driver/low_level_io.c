...
/* Shared printf mutex — defined in libc_wrap.c, initialised by
 * printf_lock_init(). NULL until the scheduler is running. */
extern SemaphoreHandle_t printf_lock_handle(void);
...
size_t __write(int handle, const unsigned char * buffer, size_t size)
{
	...
	* for all other file handles it returns failure. */
	...
	/* Serialize UART output: prevent character-level interleaving
	 * from concurrent tasks that call printf / rt_printf directly. */
	{
		SemaphoreHandle_t mtx = printf_lock_handle();
		if (mtx != NULL) { xSemaphoreTake(mtx, portMAX_DELAY); }
	for (/* Empty */; size != 0; --size)
	{
		if(*buffer=='\n')
			stdio_port_putc('\r');
