...
restart_http_ota:
	...
	if(NewFWAddr != ~0x0){
		...
		got_header_size = read_bytes >= 32 ? 32 : read_bytes;
		read_bytes -= got_header_size;
		memcpy(&header, buf+rsp_result.header_len, got_header_size);
		if(got_header_size < 32){
			read_bytes = read(server_socket, &header + got_header_size, sizeof(header) - got_header_size);
			if(read_bytes <= 0){
				printf("\n\r[%s] Read socket failed or socket closed", __FUNCTION__);
				goto update_ota_exit;
			}
			...
			memcpy(buf, buf+rsp_result.header_len+got_header_size, read_bytes);
