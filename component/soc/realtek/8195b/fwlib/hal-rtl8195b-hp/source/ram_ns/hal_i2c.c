...
hal_status_t hal_i2c_pin_register(hal_i2c_adapter_t *phal_i2c_adapter)
{
    ...
    pin_list = (io_pin_t *)&(__rom_stubs_hal_i2c.hal_i2c_pin_list[phal_i2c_adapter->init_dat.index*3 + phal_i2c_adapter->pltf_dat.pinmux]);

    hal_i2c_iopwr_enable(phal_i2c_adapter);

    hal_i2c_pin_init(phal_i2c_adapter);

    return hal_pinmux_register(pin_list, PID_I2C0+phal_i2c_adapter->init_dat.index);
}
...
hal_status_t hal_i2c_pin_unregister(hal_i2c_adapter_t *phal_i2c_adapter)
{
    ...
    hal_i2c_pin_deinit(phal_i2c_adapter);

    return hal_pinmux_unregister(pin_list, PID_I2C0+phal_i2c_adapter->init_dat.index);
}
...
hal_status_t hal_i2c_init (hal_i2c_adapter_t *phal_i2c_adapter)
{
    ...
    if (retv != HAL_OK) {
        ...
        retv = __rom_stubs_hal_i2c.hal_i2c_init(phal_i2c_adapter);
        if (phal_i2c_adapter->init_dat.master == I2CSlaveMode) {
            hal_i2c_reg_irq(phal_i2c_adapter, hal_i2c_slv_irq_handler_rtl8195bhp_patch);
        }
...
{
#if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
    return hal_i2c_send_poll_rtl8195bhp_patch(phal_i2c_adapter);
#else
...
{
#if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
    return hal_i2c_slv_send_poll_rtl8195bhp_patch (phal_i2c_adapter);
#else
...
{
    hal_irq_disable (I2C_IRQn);
    __ISB();

    // Register I2C common IRQ handler
    hal_irq_set_vector (I2C_IRQn, (uint32_t)handler);
    hal_irq_clear_pending (I2C_IRQn);
    hal_irq_enable (I2C_IRQn);
}
...
uint8_t hal_i2c_unreg_irq (hal_i2c_adapter_t *phal_i2c_adapter)
{
    ...
    if (i == (HP_I2C_NO + 1)) {
        ...
        hal_irq_disable (I2C_IRQn);
        __ISB();
        hal_irq_set_vector (I2C_IRQn, (uint32_t)NULL);
...
hal_status_t hal_i2c_send_dma (hal_i2c_adapter_t *phal_i2c_adapter)
{
    ...
    if (is_dcache_enabled ()) {
        if (phal_i2c_adapter->dcache_clean_by_addr != NULL) {
            ...
            DBG_ADC_WARN("D-Cache is enabled but clean function is NOT available before tx GDMA starts.\n");
        }
    }
...
hal_status_t hal_i2c_recv_dma (hal_i2c_adapter_t *phal_i2c_adapter)
{
    ...
    if (is_dcache_enabled ()) {
        if (phal_i2c_adapter->dcache_clean_invalidate_by_addr != NULL) {
            ...
            DBG_ADC_WARN("D-Cache is enabled but clean_invalidate function is NOT available before rx GDMA starts.\n");
        }
    }
...
uint32_t hal_i2c_slv_send_dma (hal_i2c_adapter_t *phal_i2c_adapter)
{
    ...
    if (is_dcache_enabled ()) {
        if (phal_i2c_adapter->dcache_clean_by_addr != NULL) {
            ...
            DBG_ADC_WARN("D-Cache is enabled but clean function is NOT available before slv tx GDMA starts.\n");
        }
    }
    hal_gdma_transfer_start((void *)phal_i2c_adapter->tx_dma_dat.padaptor, multi_block_en);
    return 0;
...
uint32_t hal_i2c_slv_recv_dma (hal_i2c_adapter_t *phal_i2c_adapter)
{
    ...
    if (is_dcache_enabled ()) {
        if (phal_i2c_adapter->dcache_clean_invalidate_by_addr != NULL) {
            ...
            DBG_ADC_WARN("D-Cache is enabled but clean_invalidate function is NOT available before slv rx GDMA starts.\n");
        }
    }
...
{
    //I2C0_Type *pi2c_reg = phal_i2c_adapter->init_dat.reg_base;
    uint32_t cap_l_cnt;
    uint32_t cap_l_tmp;
    uint8_t cap_m_cnt;
    uint8_t cap_m_tmp;
    ...
    /* if it's Fast/Standard mode, enable all RC and retrun */
    if ((phal_i2c_adapter->init_dat.spd_mod == I2CStandardSpeed) || (phal_i2c_adapter->init_dat.spd_mod == I2CFastSpeed)) {
        ...
        HAL_I2C_WRITE32(phal_i2c_adapter->init_dat.index, reg_offset, 0xFFFFFFFF);
        HAL_I2C_WRITE32(phal_i2c_adapter->init_dat.index, (reg_offset+0x10), 0xFFFFFFFF);
        HAL_I2C_WRITE32(phal_i2c_adapter->init_dat.index, (reg_offset+0x0C), 0xFFFFFFFF);
        DBG_I2C_ERR("Not find a good RC match at all.\n");
        DBG_I2C_ERR("Use a maximum setup\n");
        return 1;
    }


    /* calibrate for data pin */
    for (rsts_m_tmp = 0, rsts_m_cnt = 0; rsts_m_cnt <= HP_I2C_FLTR_RSTS_M_LEN; rsts_m_cnt++) {
        if (rsts_m_cnt) {
            rsts_m_tmp |= (BIT0 << (rsts_m_cnt - 1));
        }
        
        HAL_I2C_WRITE32(phal_i2c_adapter->init_dat.index, reg_offset, rsts_m_tmp);
        /* read rsts_m after write for check */
        HAL_I2C_READ32(phal_i2c_adapter->init_dat.index, reg_offset);

        for (cap_m_tmp = 0, cap_m_cnt = 0; cap_m_cnt <= HP_I2C_FLTR_CAP_M_LEN; cap_m_cnt++) {
            if (cap_m_cnt) {
                cap_m_tmp |= (BIT0 << (cap_m_cnt - 1));
            }
            
            HAL_I2C_WRITE32(phal_i2c_adapter->init_dat.index, (reg_offset+0x10), cap_m_tmp);
            /* read cap_m after write for check */
            HAL_I2C_READ32(phal_i2c_adapter->init_dat.index, reg_offset+0x10);

            for (cap_l_tmp = 0, cap_l_cnt = 0; cap_l_cnt <= HP_I2C_FLTR_CAP_L_LEN; cap_l_cnt++) {
                if (cap_l_cnt) {
                    cap_l_tmp |= (BIT0 << (cap_l_cnt - 1));
                }
                
                HAL_I2C_WRITE32(phal_i2c_adapter->init_dat.index, (reg_offset+0x0C), cap_l_tmp);
                /* read cap_l after write for check */
                HAL_I2C_READ32(phal_i2c_adapter->init_dat.index, reg_offset+0x0C);
    ...
    hal_i2c_en_ctrl(phal_i2c_adapter, I2CDisable);
    hal_i2c_en_ctrl(phal_i2c_adapter, I2CEnable);
    if ((rsts_m_cnt == HP_I2C_FLTR_RSTS_M_LEN) && (cap_m_cnt == HP_I2C_FLTR_CAP_M_LEN)
        && (cap_l_cnt == HP_I2C_FLTR_CAP_L_LEN)) {
        DBG_I2C_ERR("Not find a good RC match at all.\n");
        if (rsts_m_best != 0) {
            DBG_I2C_ERR("Use the best RC match that we could find.\n");
            //HAL_I2C_WRITE32(reg_offset, rsts_m_best);
            HAL_I2C_WRITE32(phal_i2c_adapter->init_dat.index, (reg_offset+0x10), cap_m_best);
            HAL_I2C_WRITE32(phal_i2c_adapter->init_dat.index, (reg_offset+0x0C), cap_l_best);
            DBG_I2C_INFO("R: %08x, C_m: %08x, C_l: %08x\n", HAL_I2C_READ32(phal_i2c_adapter->init_dat.index, reg_offset),
                                            HAL_I2C_READ32(phal_i2c_adapter->init_dat.index, reg_offset+0x10),
                                            HAL_I2C_READ32(phal_i2c_adapter->init_dat.index, reg_offset+0x0C));
            return 1;
