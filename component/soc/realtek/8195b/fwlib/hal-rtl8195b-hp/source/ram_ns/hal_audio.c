 ...
 *  @param[in] audio_codec_power Select audio codec LDO power.
 *
 *  @returns void
 */
void hal_audio_init (hal_audio_adapter_t *paudio_adapter, u8 audio_codec_power)
{
...
{
    audio_codec_index_04h_t audio_codec_index_04h;

    audio_codec_index_04h.w = hal_audio_si_read(paudio_adapter, 0x04); 
    //dbg_printf("micbias_count: %d \r\n", audio_codec_index_04h.b.micbias_count);
    if (fun_en == 1) {
        audio_codec_index_04h.b.micbias_count = 8;
    } else {
        audio_codec_index_04h.b.micbias_count = 0;
    }
    hal_audio_si_write(paudio_adapter, 0x04, audio_codec_index_04h.w);

    audio_codec_index_04h.w = hal_audio_si_read(paudio_adapter, 0x04); 
    //dbg_printf("micbias_count_fix: %d \r\n", audio_codec_index_04h.b.micbias_count);
