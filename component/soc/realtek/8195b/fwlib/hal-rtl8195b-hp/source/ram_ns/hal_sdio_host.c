...
void hal_sdioh_deinit (hal_sdio_host_adapter_t *psdioh_adapter)
{
    ...
    //SYSON->pad_h3l1_b.gpiob_h3l1_sel = 1;  // select 3.3V pad
    //SYSON->hs_ldoio_ctrl0_b.ldoio_bypass_en = 1;  // Bypass mode
    memset(psdioh_adapter,0,sizeof(hal_sdio_host_adapter_t));
 ...
 *  @returns    The result.
 */
extern hal_status_t hal_sdioh_read_data_rtl8195bhp_patch (hal_sdio_host_adapter_t *psdioh_adapter, u64 start_addr, u16 blk_cnt, u8 *rbuf_32align);

hal_status_t hal_sdioh_read_data (hal_sdio_host_adapter_t *psdioh_adapter, u64 start_addr, u16 blk_cnt, u8 *rbuf_32align)
{
    #if (((CHIP_VER == CHIP_A_CUT)||(CHIP_VER == CHIP_B_CUT)||(CHIP_VER == CHIP_C_CUT)) && (defined(CONFIG_BUILD_RAM)))
    return hal_sdioh_read_data_rtl8195bhp_patch(psdioh_adapter, start_addr, blk_cnt, rbuf_32align);
    #else
