...
hal_status_t spic_init(phal_spic_adaptor_t phal_spic_adaptor, u8 spic_bit_mode, u8 io_pin_sel)
{
    ...
    /*ZBIT flash needs to enable High performance mode to run at 75MHz*/
    if (phal_spic_adaptor->flash_type == FLASH_TYPE_ZBIT) {
        if ((hal_flash_get_status(phal_spic_adaptor,(phal_spic_adaptor->cmd)->rdsr3) & 0x10) == 0) {
            hal_flash_set_status(phal_spic_adaptor, (phal_spic_adaptor->cmd)->wrsr3, hal_flash_get_status(phal_spic_adaptor,(phal_spic_adaptor->cmd)->rdsr3) | 0x10);
        }
    }
    ...
    if (spic_data != 0xFFFFFFFF) {
        if (0xFFFFFFFF == (spic_data^spic_data_inv)) {
            ...
            if (spic_verify_calibration_para() == _TRUE) {
                ...
                DBG_SPIF_ERR("Bit mode %d setting cannot work, switch back to one IO mode to re-calibrate. 0x%x\n",spic_bit_mode, spic_data);
            }
        } else {
