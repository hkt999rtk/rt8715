...
void hal_flash_set_dummy_cycle (phal_spic_adaptor_t phal_spic_adaptor)
{
    ...
    if ((flash_type == FLASH_TYPE_WINBOND) 
        ...
        if ((spic_bit_mode == SpicQpiMode) || (spic_bit_mode == SpicQpiDtrMode)) {
            /*128 Mbit ZBIT flash, dummy cycle is different from others*/
            if ((flash_type == FLASH_TYPE_ZBIT) 
                && (phal_spic_adaptor->flash_id[2] == 0x18)) {                
                dummy_para = 0x10;
            }
                
            spic_tx_cmd_no_check(phal_spic_adaptor, 0xC0, 1, &dummy_para);
        }
    }
}
...
void hal_flash_enable_qpi (phal_spic_adaptor_t phal_spic_adaptor)
{
    ...
    spic_bit_mode = phal_spic_adaptor->spic_bit_mode;

    switch (flash_type) {
        case FLASH_TYPE_ADESTO:
            if (SpicQpiMode == spic_bit_mode) {
                status_value = hal_flash_get_status_with_addr(phal_spic_adaptor, cmd->rdsr2, 0x2);
                hal_flash_set_status_no_check(phal_spic_adaptor, cmd->wrsr2, 0x4 | status_value);
            } else {
            /*QPI DTR Mode*/
                status_value = hal_flash_get_status_with_addr(phal_spic_adaptor, cmd->rdsr2, 0x2);
                hal_flash_set_status_no_check(phal_spic_adaptor, cmd->wrsr2, 0x84 | status_value);
            }
            break;
...
/** \brief Description of hal_flash_enable_opi
 *
 *    hal_flash_enable_opi is used to switch flash to OPI mode (8-8-8).
 ...
 *   \return void.
 */
void hal_flash_enable_opi (phal_spic_adaptor_t phal_spic_adaptor)
{
    hal_flash_stubs.hal_flash_enable_opi(phal_spic_adaptor);
}
...
void hal_flash_return_spi (phal_spic_adaptor_t phal_spic_adaptor)
{
    ...
    if ((spic_send_cmd_mode == QuadChnl) || (spic_send_cmd_mode == OctalChnl)) {
        if (FLASH_TYPE_ADESTO == flash_type) {
            spic_tx_cmd_no_check(phal_spic_adaptor, cmd->wren, 0, 0);
        }
...
/** \brief Description of hal_flash_enable_opi_dtr
 *
 *    hal_flash_enable_opi_dtr is used to set flash to OPI DTR mode.
 ...
 *   \return void.
 */
void hal_flash_enable_opi_dtr (phal_spic_adaptor_t phal_spic_adaptor)
{
    hal_flash_stubs.hal_flash_enable_opi_dtr(phal_spic_adaptor);
}

/** \brief Description of hal_flash_disable_dtr
 *
 *    hal_flash_disable_dtr is used to set octal flash to STR mode.
 ...
 *   \return void.
 */
void hal_flash_disable_dtr (phal_spic_adaptor_t phal_spic_adaptor)
{
    hal_flash_stubs.hal_flash_disable_dtr(phal_spic_adaptor);
}
...
void hal_flash_read_write_flash (phal_spic_adaptor_t phal_spic_adaptor, u32 length, u32 addr, u8 *data)
{
    ...
    u8 addr_byte_num;
    u8 spic_bit_mode;
    u8 upper_cmd = (cmd->pp & 0xF0) >> 4;
    u8 lower_cmd = (cmd->pp & 0x0F);
    u8 buffer[256];
    u8* ptr = &buffer[0];
    ...
    if (FLASH_TYPE_ADESTO != flash_type) {
        if (phal_spic_adaptor->dtr_en) {
            ...
            cmd_ddr_en = ctrlr0.b.cmd_ddr_en;
            addr_ddr_en = ctrlr0.b.addr_ddr_en;
            data_ddr_en = ctrlr0.b.data_ddr_en;

            /*Should program under STR mode*/
            ctrlr0.b.cmd_ddr_en = DISABLE;
            ctrlr0.b.addr_ddr_en = DISABLE;
            ctrlr0.b.data_ddr_en = DISABLE;
        }
    }
    ...
    if (FLASH_TYPE_ADESTO == flash_type) {
        if (SpicOpiDtrMode == spic_bit_mode) {
            spic_dev->dr_half_word = cmd->pp | (cmd->pp << 8);
        } else if (SpicQpiDtrMode == spic_bit_mode) {
            spic_dev->dr_half_word = upper_cmd | (upper_cmd << 4) | (lower_cmd << 8) | (lower_cmd << 12);
        } else {
    ...
    switch (spic_send_cmd_mode) {
        ...
        case OctalChnl:
            ctrlr0.b.cmd_ch = OctalChnl;
            ctrlr0.b.addr_ch = OctalChnl;
            ctrlr0.b.data_ch = OctalChnl;
            break;
    ...
    if (FLASH_TYPE_ADESTO != flash_type) {
        if (phal_spic_adaptor->dtr_en) {
            ctrlr0.w = spic_dev->ctrlr0;
            ctrlr0.b.cmd_ddr_en = cmd_ddr_en;
            ctrlr0.b.addr_ddr_en = addr_ddr_en;
            ctrlr0.b.data_ddr_en = data_ddr_en;
            spic_dev->ctrlr0 = ctrlr0.w;
        }
    }
}
    ...
    switch (flash_id) {
        ...
        case 0x0B:
            ...
            if (phal_spic_adaptor->spic_bit_mode == SpicQpiMode) {
                phal_spic_adaptor->cmd->str_4read = 0x0B;
            } else {              
                phal_spic_adaptor->cmd->str_4read = 0xEB;
            }
            break;
            
        case 0x46:
            phal_spic_adaptor->cmd = &new_flash_cmd;
            phal_spic_adaptor->cmd->en_qpi = 0x35;
            phal_spic_adaptor->cmd->en_spi = 0xF5;
            phal_spic_adaptor->flash_type = FLASH_TYPE_XMC;
            break;
            
        case 0x1C:
            phal_spic_adaptor->cmd = &new_flash_cmd;
            phal_spic_adaptor->cmd->rdsr2 = 0x09;
            phal_spic_adaptor->cmd->rdsr3 = 0x95;
            phal_spic_adaptor->cmd->wrsr3 = 0xC0;
