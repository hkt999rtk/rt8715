...
{
    if ((full_lvl == 0) || (full_lvl > LP_ADC_FIFO_LEN)) {
...
#else
    return hal_adc_stubs.hal_adc_read_fifo(phal_adc_adapter, out_raw);
#endif
}
/** \brief Description of hal_adc_read_fifo_w_sts
 *
 *    hal_adc_read_fifo_w_sts is used to read the fifo register(global) with its
 *    status bits (channel, ready, overwrite).
 *
 *   \param[in] hal_adc_adapter_t *phal_adc_adapter:          Pointer to ADC control adapter.
 *   \return uint32_t:                              Global data register content.
 */
...
/** \brief Description of hal_adc_set_pwr_cut
 *
 *    hal_adc_get_cv_intr_ch is used to set ADC internal power control bit
 *
 *   \param[in] hal_adc_adapter_t *phal_adc_adapter:          Pointer to ADC control adapter.
 */
__STATIC_INLINE void hal_adc_set_pwr_cut (hal_adc_adapter_t *phal_adc_adapter)
{
    phal_adc_adapter->init_dat.reg_base->ana_ctrl_b.pwr_cut = ADCEnable;
}

/** \brief Description of hal_adc_clr_pwr_cut
 *
 *    hal_adc_get_cv_intr_ch is used to clear ADC internal power control bit
 *
 *   \param[in] hal_adc_adapter_t *phal_adc_adapter:          Pointer to ADC control adapter.
 */
__STATIC_INLINE void hal_adc_clr_pwr_cut (hal_adc_adapter_t *phal_adc_adapter)
{
    phal_adc_adapter->init_dat.reg_base->ana_ctrl_b.pwr_cut = ADCDisable;
}

__STATIC_INLINE void hal_adc_enable (hal_adc_adapter_t *phal_adc_adapter, uint8_t enable)
{
    ADC_Type *padc_reg = (ADC_Type *)phal_adc_adapter->init_dat.reg_base;
    padc_reg->conf_b.en = ADCEnable;
}
