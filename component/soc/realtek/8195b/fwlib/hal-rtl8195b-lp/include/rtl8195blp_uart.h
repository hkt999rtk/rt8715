...
#ifndef _RTL8195BLP_UART_H_
#define _RTL8195BLP_UART_H_
...
#include "rtl8195blp_uart_type.h"

/// The number of UART port on this platform
#define MAX_UART_PORT                   2

/// the minimum OVSR value for baud rate setting calculation. 1000 = 10.0
#define UART_OVSR_POOL_MIN      1000    // 10.0
/// the maximum OVSR value for baud rate setting calculation. 2090 = 20.90
#define UART_OVSR_POOL_MAX      2090    //20.9
...
/// the SCLK value for the UART hardware IP
#define UART_SCLK               PLATFORM_CLK
...
typedef struct hal_uart_adapter_s {
    ...
    uint8_t pin_mux_sel;        /*!< the pin mux selection */
...
typedef struct hal_uart_defconfig_s {
    ...
    uint8_t stop_bit;               /*!< stop bit length selection: 0 -> 1 stop bit, 1 -> 2 stop bit */
    uint8_t parity_type;            /*!< parity check type: 0:Odd, 1: Even */
    irq_handler_t irq_handle;       /*!< the IRQ register/unregister */
...
/** 
 * @addtogroup ls_hal_uart_rom_func UART HAL ROM APIs.
 * @{
 */
 ...
 *  @returns    void.
 */ 
__STATIC_INLINE void uart_clear_state_rtl8195blp(phal_uart_adapter_t puart_adapter, uint32_t state)
{
    puart_adapter->state &= ~state;
    __DSB();
}

void hal_uart_enter_critical_rtl8195blp (void);
void hal_uart_exit_critical_rtl8195blp (void);
void hal_uart_func_clk_ctrl_rtl8195blp (uint8_t uart_idx, BOOL en);
hal_status_t hal_uart_reset_rx_fifo_rtl8195blp (phal_uart_adapter_t puart_adapter);
hal_status_t hal_uart_gen_baudrate_rtl8195blp (uart_speed_setting_t *pbaud_setting);
hal_status_t hal_uart_set_baudrate_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t baudrate);
hal_status_t hal_uart_set_format_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t data_bits, 
                                            uint32_t parity, uint32_t stop_bits);
hal_status_t hal_uart_set_flow_control_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t flow_ctrl);
void hal_uart_comm_init_rtl8195blp (hal_uart_group_adapter_t *puart_comm_adapter);
hal_status_t hal_uart_init_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t uart_idx, 
                                        phal_uart_defconfig_t pconfig);
void hal_uart_deinit_rtl8195blp (phal_uart_adapter_t puart_adapter);
hal_status_t hal_uart_tx_gdma_init_rtl8195blp(phal_uart_adapter_t puart_adapter, phal_gdma_adaptor_t pgdma_chnl);
hal_status_t hal_uart_tx_gdma_deinit_rtl8195blp(phal_uart_adapter_t puart_adapter);
hal_status_t hal_uart_rx_gdma_init_rtl8195blp(phal_uart_adapter_t puart_adapter, phal_gdma_adaptor_t pgdma_chnl);
hal_status_t hal_uart_rx_gdma_deinit_rtl8195blp(phal_uart_adapter_t puart_adapter);
BOOL hal_uart_writeable_rtl8195blp (phal_uart_adapter_t puart_adapter);
void hal_uart_putc_rtl8195blp (phal_uart_adapter_t puart_adapter, uint8_t tx_data);
void hal_uart_wputc_rtl8195blp (phal_uart_adapter_t puart_adapter, uint8_t tx_data);
void hal_uart_wait_tx_done_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t timeout_ms);
uint32_t hal_uart_send_rtl8195blp (phal_uart_adapter_t puart_adapter, uint8_t *ptx_buf, uint32_t len, uint32_t timeout_ms);
hal_status_t hal_uart_int_send_rtl8195blp (phal_uart_adapter_t puart_adapter, uint8_t *ptx_buf, uint32_t len);
hal_status_t hal_uart_dma_send_rtl8195blp (phal_uart_adapter_t puart_adapter, uint8_t *ptx_buf, uint32_t len);
hal_status_t hal_uart_send_abort_rtl8195blp (phal_uart_adapter_t puart_adapter);
BOOL hal_uart_readable_rtl8195blp (phal_uart_adapter_t puart_adapter);
char hal_uart_getc_rtl8195blp (phal_uart_adapter_t puart_adapter);
int hal_uart_rgetc_rtl8195blp (phal_uart_adapter_t puart_adapter, char *data);
uint32_t hal_uart_recv_rtl8195blp (phal_uart_adapter_t puart_adapter, uint8_t *prx_buf, uint32_t len, uint32_t timeout_ms);
hal_status_t hal_uart_int_recv_rtl8195blp (phal_uart_adapter_t puart_adapter, uint8_t *prx_buf, uint32_t len);
hal_status_t hal_uart_dma_recv_rtl8195blp (phal_uart_adapter_t puart_adapter, uint8_t *prx_buf, uint32_t len);
uint32_t hal_uart_recv_abort_rtl8195blp (phal_uart_adapter_t puart_adapter);
uint8_t hal_uart_get_imr_rtl8195blp (phal_uart_adapter_t puart_adapter);
void hal_uart_set_imr_rtl8195blp (phal_uart_adapter_t puart_adapter);
void hal_uart_set_rts_rtl8195blp (phal_uart_adapter_t puart_adapter, BOOLEAN rts_ctrl);
void hal_uart_tx_pause_rtl8195blp (phal_uart_adapter_t puart_adapter, BOOLEAN cts_ctrl);
void hal_uart_reg_comm_irq_rtl8195blp (irq_handler_t handler);
void hal_uart_reg_irq_rtl8195blp (phal_uart_adapter_t puart_adapter, irq_handler_t handler);
void hal_uart_unreg_irq_rtl8195blp (phal_uart_adapter_t puart_adapter);
void hal_uart_adapter_init_rtl8195blp (phal_uart_adapter_t puart_adapter, uint8_t uart_idx, phal_uart_defconfig_t pconfig);
void hal_uart_line_sts_hook_rtl8195blp (phal_uart_adapter_t puart_adapter, uart_lsr_callback_t pcallback, void *pdata);
void hal_uart_txtd_hook_rtl8195blp (phal_uart_adapter_t puart_adapter, uart_irq_callback_t pcallback, uint32_t id, uint32_t event);
void hal_uart_rxind_hook_rtl8195blp (phal_uart_adapter_t puart_adapter, uart_irq_callback_t pcallback, uint32_t id, uint32_t event);
void hal_uart_txdone_hook_rtl8195blp (phal_uart_adapter_t puart_adapter, uart_callback_t pcallback, void *parg);
void hal_uart_rxdone_hook_rtl8195blp (phal_uart_adapter_t puart_adapter, uart_callback_t pcallback, void *parg);
void hal_uart_set_rx_filter_pattern_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t pattern, 
                                                uint32_t mask, uint32_t mask_en);
void hal_uart_set_rx_filter_op_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t pattern_len, uint32_t match_op);
void hal_uart_set_rx_filter_timeout_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t timeout_ms, 
                                                uart_callback_t callback, void *cb_arg);
void hal_uart_rx_filter_en_rtl8195blp (phal_uart_adapter_t puart_adapter, uart_callback_t callback, void *cb_arg);
void hal_uart_rx_filter_dis_rtl8195blp (phal_uart_adapter_t puart_adapter);
void hal_uart_reset_receiver_rtl8195blp (phal_uart_adapter_t puart_adapter);
void hal_uart_set_tx_fifo_level_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t low_level);
void hal_uart_set_rx_fifo_level_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t level);
void hal_uart_rx_idle_timeout_en_rtl8195blp (phal_uart_adapter_t puart_adapter, uint32_t timeout_us,
                                            uart_callback_t pcallback, void *parg);
void hal_uart_rx_idle_timeout_dis_rtl8195blp (phal_uart_adapter_t puart_adapter);
void hal_uart_tx_fifo_low_hook_rtl8195blp (phal_uart_adapter_t puart_adapter, uart_callback_t pcallback,
                                                        void *parg);

/** @} */ /* End of group ls_hal_uart_rom_func */
...
typedef struct hal_uart_func_stubs_s {
    ...
    hal_status_t (*hal_uart_init) (phal_uart_adapter_t puart_adapter, uint32_t uart_idx, phal_uart_defconfig_t pconfig);
    ...
    void (*hal_uart_txtd_hook) (phal_uart_adapter_t puart_adapter, uart_irq_callback_t pcallback, uint32_t id, uint32_t ev);
    void (*hal_uart_rxind_hook) (phal_uart_adapter_t puart_adapter, uart_irq_callback_t pcallback, uint32_t id, uint32_t ev);
    ...
    // following functions are added for B-cut ROM
...
#endif  // #ifndef _RTL8195BL_UART_H_
