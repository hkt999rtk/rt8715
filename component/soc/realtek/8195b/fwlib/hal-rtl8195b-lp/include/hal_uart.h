 ...
 * @ingroup 8195bl_hal
 * @{
 * @brief The UART HAL module of the LS platform.
 */
 ...
 *  @returns    Always return HAL_OK
 */ 
__STATIC_INLINE hal_status_t hal_uart_set_flow_control (phal_uart_adapter_t puart_adapter, uint32_t flow_ctrl)
{
...
/** 
 *  @brief To initial a GDMA channel for UART TX.
 *
 *  @param puart_adapter[in] The UART adapter.
 *  @param pgdma_chnl[in] The GDMA adapter for the GDMA channel handling.
 *
 *  @returns The result. HAL_ERR_PARA or HAL_OK.
 */
__STATIC_INLINE hal_status_t hal_uart_tx_gdma_init(phal_uart_adapter_t puart_adapter, phal_gdma_adaptor_t pgdma_chnl)
{
    return hal_uart_stubs.hal_uart_tx_gdma_init(puart_adapter, pgdma_chnl);
}
 ...
 */
__STATIC_INLINE hal_status_t hal_uart_tx_gdma_deinit(phal_uart_adapter_t puart_adapter)
{
 ...
 */
__STATIC_INLINE hal_status_t hal_uart_rx_gdma_init(phal_uart_adapter_t puart_adapter, phal_gdma_adaptor_t pgdma_chnl)
{
    return hal_uart_stubs.hal_uart_rx_gdma_init(puart_adapter, pgdma_chnl);
}
 ...
 *  @return     HAL_OK:  GDMA channel de-initialization OK.
 */
__STATIC_INLINE hal_status_t hal_uart_rx_gdma_deinit(phal_uart_adapter_t puart_adapter)
{
 ...
 */
__STATIC_INLINE hal_status_t hal_uart_dma_send (phal_uart_adapter_t puart_adapter, uint8_t *ptx_buf, uint32_t len)
{
...
/** 
 *  @brief To check is a char ready in RX FIFO.
 *
 *  @param[in]  puart_adapter  The UART adapter.
 *
 *  @returns    Return 1 if at least 1 byte of data is ready in the RX FIFO, 
 *              otherwise return 0.
 */
 ...
 *  @return     HAL_NO_RESOURCE: Multiple-block DMA channel allocation failed.
 */
__STATIC_INLINE hal_status_t hal_uart_dma_recv (phal_uart_adapter_t puart_adapter, uint8_t *prx_buf, uint32_t len)
{
...
{
    hal_uart_stubs.hal_uart_set_rts (puart_adapter, rts_ctrl);
}
...
void hal_uart_rx_filter_dis (phal_uart_adapter_t puart_adapter)
{
    ...
    puart_adapter->base_addr->vier_b.rf_match_int_en = 0;   // disable RX filter matched interrupt
}
