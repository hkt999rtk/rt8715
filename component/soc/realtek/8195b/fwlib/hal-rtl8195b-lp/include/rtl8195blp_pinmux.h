/**************************************************************************//**
 * @file     rtl8195bl_pinmux.h
 * @brief    Define the pin mux management API
 * @version  V1.00
 * @date     2016-12-31
 *
 * @note
...
#ifndef RTL8195BLP_PIN_MUX_H
#define RTL8195BLP_PIN_MUX_H
...
/**
  \brief  The data structure to handle Pin Mux conflct, validate, power down management functions.
*/
typedef struct hal_pin_mux_mang_s {
    ...
    uint16_t pinmux_reg_log;      // the bit map to record each pin is in using or not
    uint16_t pinmux_pwr_log;      // the bit map to record each pin is powered(enabled) or not
...
void hal_pinmux_manager_init_rtl8195blp (hal_pin_mux_mang_t *pinmux_manag);
hal_status_t hal_pin_register_rtl8195blp (io_pin_t *pin_list, uint8_t periphl_id);
hal_status_t hal_pin_unregister_rtl8195blp (io_pin_t *pin_list, uint8_t periphl_id);
hal_status_t hal_pin_pwrup_rtl8195blp (io_pin_t *pin_list, uint8_t periphl_id);
hal_status_t hal_pin_pwrdwn_rtl8195blp (io_pin_t *pin_list, uint8_t periphl_id);
hal_status_t hal_pinmux_register_rtl8195blp (io_pin_t *pin_list, uint8_t periphl_id);
hal_status_t hal_pinmux_unregister_rtl8195blp (io_pin_t *pin_list, uint8_t periphl_id);
