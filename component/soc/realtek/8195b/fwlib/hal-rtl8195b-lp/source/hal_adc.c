...
uint16_t hal_adc_single_read (hal_adc_adapter_t *phal_adc_adapter, uint8_t ch_no)
{
    ...
    if (phal_adc_adapter->init_dat.hw_mode == ADCAutoMod) {
        ...
        if (ch_no < 7) {
            ...
            hal_adc_read_cali_param(LP_ADC_BAT_G_DENO_ADDR, (uint8_t *)&adc_cali_paras_tmp.gain_deno, 2);
            hal_adc_read_cali_param(LP_ADC_BAT_G_MOLE_ADDR, (uint8_t *)&adc_cali_paras_tmp.gain_mole, 2);
            hal_adc_read_cali_param(LP_ADC_BAT_O_DENO_ADDR, (uint8_t *)&adc_cali_paras_tmp.offset_deno, 4);
            hal_adc_read_cali_param(LP_ADC_BAT_O_MOLE_ADDR, (uint8_t *)&adc_cali_paras_tmp.offset_mole, 2);
        } else {
            adc_cali_paras_tmp.gain_deno = 0xFFFF;
            adc_cali_paras_tmp.gain_mole= 0xFFFF;
            adc_cali_paras_tmp.offset_deno = 0xFFFFFFFF;
            adc_cali_paras_tmp.offset_mole = 0xFFFF;
        }
...
uint16_t hal_adc_calc_real_val(uint16_t adc_ideal_val, hal_adc_cali_para_t *phal_adc_cali_para) 
{
    ...
    if ((phal_adc_cali_para->gain_deno == (int16_t)0xFFFF) || (phal_adc_cali_para->gain_mole == (int16_t)0xFFFF) || 
        (phal_adc_cali_para->offset_deno == (int32_t)0xFFFFFFFF) || (phal_adc_cali_para->offset_mole == (int16_t)0xFFFF)) {
