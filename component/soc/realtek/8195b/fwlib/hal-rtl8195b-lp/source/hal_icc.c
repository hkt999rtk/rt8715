...
hal_icc_adapter_t *pgicc_adapter;
...
/** 
  * @brief The stubs functions table to exports ICC HAL functions in ROM.
  */
                                   ...
                                   uint32_t cmd_word1, uint32_t timeout_us)
{
    ...
    return hal_icc_l2h_cmd_send_rtl8195blp_patch (icc_adapter, cmd_word0, cmd_word1, timeout_us);
#else
    return hal_icc_stubs.hal_icc_l2h_cmd_send (icc_adapter, cmd_word0, cmd_word1, timeout_us);
#endif
}

/** 
 *  @brief Resets the specified message queue.
 *
...
/** 
 *  @brief Hooks a callback function for the interrupt of HS to LS command indication.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
{
    hal_icc_stubs.hal_icc_h2l_cmd_ind_hook (icc_adapter, pcallback, pdata);
}

/** 
 *  @brief Hooks a callback function for the interrupt of HS to LS message queue is updated.
 *         This callback function will be called when the HS platform fills new data 
...
{
    hal_icc_stubs.hal_icc_h2l_queue_ind_hook (icc_adapter, pcallback, pdata);
}

/** 
 *  @brief Hooks a callback function for the interrupt of LS to HS message queue is acknowledged.
 *         This callback function will be called when the HS platform reads data from the message
...
{
    hal_icc_stubs.hal_icc_l2h_queue_ack_hook (icc_adapter, pcallback, pdata);
}
...
void hal_icc_init (hal_icc_adapter_t *icc_adapter)
{
    ...
    icc_adapter->enter_critical = hal_icc_stubs.hal_icc_enter_critical;
    icc_adapter->exit_critical = hal_icc_stubs.hal_icc_exit_critical;
    
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    icc_adapter->l2h_msg_q_ack = _icc_l2h_q_ack_handler_patch;
    icc_adapter->l2h_msg_q_ack_cb_para = icc_adapter;
    icc_adapter->h2l_msg_q_ind = _icc_h2l_q_ind_handler_patch;
    icc_adapter->h2l_msg_q_ind_cb_para = icc_adapter;
    L2H_HQ_ACK_IE = 0;
    pgicc_adapter = icc_adapter;
    ...
    icc_adapter->cmd_q_sn = H2L_CMD_TG;
...
/** 
 *  @brief Pushs(send) a block of data into the LS to HS message queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
{
    return hal_icc_stubs.hal_icc_push_l2hq (icc_adapter, q_id_size, pbuf, timeout_us);
}

/** 
 *  @brief Pulls(receives) a block of data from the HS to LS message queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
{
    return hal_icc_stubs.hal_icc_pull_h2lq (icc_adapter, q_id, pbuf, size);
}
...
/** 
 *  @brief Pushs(send) a block of data into the LS to HS message Hi-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  pbuf  The data buffer to be send.
...
{
    return hal_icc_stubs.hal_icc_push_l2hq (icc_adapter, (QID_L2H_HQ << 24) | (size & 0x00FFFFFF), pbuf, timeout_us);
}

/** 
 *  @brief Pushs(send) a block of data into the LS to HS message Lo-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  pbuf  The data buffer to be send.
 *  @param[in]  size  The size of the data to be pushed, in byte.
...
hal_status_t hal_icc_push_l2h_lq (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf, 
                                            uint32_t size, uint32_t timeout_us)
{
    return hal_icc_stubs.hal_icc_push_l2hq (icc_adapter, (QID_L2H_LQ << 24) | (size & 0x00FFFFFF), pbuf, timeout_us);
}

/** 
 *  @brief Pulls(receives) a block of data from the HS to LS message Hi-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  pbuf  The data buffer for the data reading from the message queue.
 *  @param[in]  size  The maximum data length to receive(the buffer size), in byte.
...
{
    return hal_icc_stubs.hal_icc_pull_h2lq (icc_adapter, QID_H2L_HQ, pbuf, size);
}

/** 
 *  @brief Pulls(receives) a block of data from the HS to LS message Lo-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  pbuf  The data buffer for the data reading from the message queue.
 *  @param[in]  size  The maximum data length to receive(the buffer size), in byte.
...
{
    return hal_icc_stubs.hal_icc_pull_h2lq (icc_adapter, QID_H2L_LQ, pbuf, size);
}

/** 
 *  @brief To send a block of data to HS via the LS to HS message Hi-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  pbuf  The data buffer to be send.
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_send_l2h_hq_rtl8195blp_patch (icc_adapter, pbuf, size, timeout_us);
#else
    return hal_icc_stubs.hal_icc_send_l2h_hq (icc_adapter, pbuf, size, timeout_us);
#endif
}

/** 
 *  @brief To send a block of data to HS via the LS to HS message Lo-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  pbuf  The data buffer to be send.
 *  @param[in]  size  The size of the data to be send, in byte.
...
hal_status_t hal_icc_send_l2h_lq (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf, uint32_t size, 
                                  uint32_t timeout_us)
{
    return hal_icc_stubs.hal_icc_send_l2h_lq (icc_adapter, pbuf, size, timeout_us);
}

/** 
 *  @brief To send a block of data to HS via the LS to HS message queue by the interrupt mode.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_send_l2hq_int_rtl8195blp_patch(icc_adapter, q_id, send_req);
#else
    return hal_icc_stubs.hal_icc_send_l2hq_int (icc_adapter, q_id, send_req);
#endif
}
...
{
    return hal_icc_stubs.hal_icc_send_l2hq_abort (icc_adapter, q_id);
}

/** 
 *  @brief Sends a block of data to the HS platform via the LS to HS message Hi-queue
...
{
    return hal_icc_stubs.hal_icc_send_l2hq_int (icc_adapter, QID_L2H_HQ, send_req);
}

/** 
 *  @brief Sends a block of data to the HS platform via the LS to HS message Lo-queue
 *         by the interrupt mode.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  send_req  The message queue send request which contains the buffer address, 
...
{
    return hal_icc_stubs.hal_icc_send_l2hq_int (icc_adapter, QID_L2H_LQ, send_req);
}

/** 
 *  @brief Aborts interrupt mode message data sending on the LS to HS Hi-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *
 *  @returns    The length of data has been transfered.
...
{
    return hal_icc_stubs.hal_icc_send_l2hq_abort (icc_adapter, QID_L2H_HQ);
}

/** 
 *  @brief Aborts interrupt mode message data sending on the LS to HS Lo-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *
 *  @returns    The length of data has been transfered.
...
{
    return hal_icc_stubs.hal_icc_send_l2hq_abort (icc_adapter, QID_L2H_LQ);
}

/** 
 *  @brief Receives a block of data from a specified HS to LS message queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
uint32_t hal_icc_recv_h2lq (hal_icc_adapter_t *icc_adapter, uint32_t q_id_size, 
                            uint8_t *pbuf, uint32_t timeout_us)
{
    return hal_icc_stubs.hal_icc_recv_h2lq (icc_adapter, q_id_size, pbuf, timeout_us);
}

/** 
 *  @brief Receives a block of data from the HS to LS message Hi-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[out]  pbuf  The buffer for the message data receiving.
...
uint32_t hal_icc_recv_h2l_hq (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf, 
                              uint32_t size, uint32_t timeout_us)
{
    return hal_icc_stubs.hal_icc_recv_h2lq (icc_adapter, (QID_H2L_HQ << 24) | (size & 0xFFFFFF) , pbuf, timeout_us);
}
...
uint32_t hal_icc_recv_h2l_lq (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf, 
                              uint32_t size, uint32_t timeout_us)
{
    return hal_icc_stubs.hal_icc_recv_h2lq (icc_adapter, (QID_H2L_LQ << 24) | (size & 0xFFFFFF) , pbuf, timeout_us);
}

/** 
 *  @brief Receives a block of data from a specified HS to LS message queue by the interrupt mode.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_recv_h2lq_int_rtl8195blp_patch(icc_adapter, q_id, rcv_req);
#else
    return hal_icc_stubs.hal_icc_recv_h2lq_int (icc_adapter, q_id, rcv_req);
#endif
}

/** 
 *  @brief Receives a block of data from the HS to LS message Hi-queue by the interrupt mode.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  rcv_req  The queue receive request which contains the buffer address, data length
 *                       and the callback for the message data receiving is done.
...
{
    return hal_icc_stubs.hal_icc_recv_h2lq_int (icc_adapter, QID_H2L_HQ, rcv_req);
}

/** 
 *  @brief Receives a block of data from the HS to LS message Lo-queue by the interrupt mode.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  rcv_req  The queue receive request which contains the buffer address, data length
 *                       and the callback for the message data receiving is done.
...
{
    return hal_icc_stubs.hal_icc_recv_h2lq_int (icc_adapter, QID_H2L_LQ, rcv_req);
}
...
{
    return hal_icc_stubs.hal_icc_recv_h2lq_abort (icc_adapter, q_id);
}

/** 
 *  @brief Abort an interrupt mode message queue data receiving on the HS to LS message Hi-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *
 *  @returns    The length of data has been received.
...
{
    return hal_icc_stubs.hal_icc_recv_h2lq_abort (icc_adapter, QID_H2L_HQ);
}

/** 
 *  @brief Abort an interrupt mode message queue data receiving on the HS to LS message Lo-queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *
 *  @returns    The length of data has been received.
...
{
    return hal_icc_stubs.hal_icc_recv_h2lq_abort (icc_adapter, QID_H2L_LQ);
}
...
/** 
 *  @brief To unregister(remove) an ICC command from the user application ICC command table.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
/** 
 *  @brief Sends a user application message with data to the HS platform via a 
 *         specified LS to HS message queue.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_send_msg_rtl8195blp_patch (icc_adapter, picc_msg, timeout_us);
#else
 ...
 *         specified LS to HS message queue by the interrupt mode.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *  @param[in]  picc_msg  The message send request. The request includes the mesage type, queue index,
 *                        the data buffer and the data size for the message sending.
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_send_msg_int_rtl8195blp_patch (icc_adapter, picc_msg, callback, callback_para);
#else
...
/** 
 *  @brief Sets the timeout period for ICC command completion waitting.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
{
    hal_icc_stubs.hal_icc_set_timeout(icc_adapter, cmd_timeout_us);
}

/** 
 *  @brief Sends a Ping command to the HS platform to check whether the HS ICC HAL
 *         is ready or not.
 *
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    hal_icc_irq_handler_bh_rtl8195blp_patch ();
#else
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    hal_icc_init_cmd_q_rtl8195blp_patch (icc_adapter, pbuf, cmd_q_size);
#else
...
/** 
 *  @brief Adds a message queue TX request to the free TX request pool.
 *  @param[in]  icc_adapter  The ICC HAL adapter.
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    hal_icc_tx_req_pool_fill_rtl8195blp_patch (icc_adapter, q_req);
#else
...
/** 
 *  @brief To takes out a message queue TX request from the free TX request pool.
 *  @param[in]  icc_adapter  The ICC HAL adapter.
 *
 *  @return     The gotton message queue TX request. If the no free TX request in the pool, returns NULL.
 */
picc_msg_tx_req_t hal_icc_tx_req_pool_get (hal_icc_adapter_t *icc_adapter)
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_tx_req_pool_get_rtl8195blp_patch (icc_adapter);
#else
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    hal_icc_msg_tx_req_pending_process_rtl8195blp_patch (icc_adapter, q_id);
#else
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_msg_tx_req_submit_rtl8195blp_patch (icc_adapter, q_req);
#else
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    icc_msg_rx_ready_handler_rtl8195blp_patch (icc_adapter, frame_type, msg_type, pmsg_data, msg_len);
#else
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_msg_rx_req_done_deq_rtl8195blp_patch (icc_adapter, q_id);
#else
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    hal_icc_msg_rx_req_done_process_rtl8195blp_patch (icc_adapter, q_id);
#else
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_msg_rx_req_submit_rtl8195blp_patch (icc_adapter, q_req);
#else
    return hal_icc_stubs.hal_icc_msg_rx_req_submit (icc_adapter, q_req);
#endif
}
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    hal_icc_msg_rx_req_pending_process_rtl8195blp_patch (icc_adapter, q_id);
#else
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    return hal_icc_msg_tbl_reg_rtl8195blp_patch(icc_adapter, msg, frame, callback, cb_arg);
#else
...
{
#if defined(CONFIG_BUILD_RAM) && (CHIP_VER == CHIP_A_CUT)
    hal_icc_msg_tbl_unreg_rtl8195blp_patch(icc_adapter, msg, frame);
#else
