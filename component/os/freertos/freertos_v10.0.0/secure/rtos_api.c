						...
						TaskHandle_t * const pxCreatedTask ) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
{
	...
	TaskHandle_t* (__cmse_nonsecure_call *wrapper)( TaskFunction_t pxTaskCode, uint16_t usSecureStackDepth, void * pvParameters, UBaseType_t uxPriority);
	wrapper = ( TaskHandle_t* (__cmse_nonsecure_call *)( TaskFunction_t , uint16_t , void * , UBaseType_t ) )rtos_ns.taskCreate;
	if(wrapper)
	#else
	TaskHandle_t* __attribute__((cmse_nonsecure_call)) (*wrapper)( TaskFunction_t pxTaskCode, uint16_t usSecureStackDepth, void * pvParameters, UBaseType_t uxPriority);
	wrapper = cmse_nsfptr_create(rtos_ns.taskCreate);
	if (wrapper && cmse_is_nsfptr(wrapper)) 
	#endif
	{
		TaskHandle_t createdTask = wrapper(pxTaskCode, usSecureStackDepth, pvParameters, uxPriority);
		if(pxCreatedTask)
		...
		if(createdTask){
			...
			strncpy(name, pcName, strlen("_secure_"));
			#endif
			return pdTRUE;
		}
		else return pdFAIL;
...
{
	#if defined(__ICCARM__)	
	BaseType_t (__cmse_nonsecure_call *wrapper)(QueueHandle_t xQueue, void * pvBuffer, TickType_t xTicksToWait, BaseType_t xJustPeeking);
	wrapper = ( BaseType_t (__cmse_nonsecure_call *)(QueueHandle_t , void * , TickType_t , BaseType_t ) )rtos_ns.queueGenericReceive;
	if(wrapper)
	#else	
	BaseType_t __attribute__((cmse_nonsecure_call)) (*wrapper)(QueueHandle_t xQueue, void * pvBuffer, TickType_t xTicksToWait, BaseType_t xJustPeeking);
	wrapper = cmse_nsfptr_create(rtos_ns.queueGenericReceive);
	if (wrapper && cmse_is_nsfptr(wrapper)) 
	#endif
	{
		return wrapper(xQueue, pvBuffer, xTicksToWait, xJustPeeking);
	}	
	return pdFAIL;
}
