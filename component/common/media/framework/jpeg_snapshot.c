...
{	
	...
	if(abs(xTaskGetTickCountFromISR() - snapshot_ctx->current_time)< JPEG_SNAPSHOT_THRESHOLD)
          is_output_ready = 0;
...
int jpeg_snapshot_isp_config(int streamid)
{
	...
	if(snapshot_ctx->isp_snapshot_ctx->stream->cfg.format == ISP_FORMAT_YUV420_SEMIPLANAR)
		isp_buffer = malloc((int)(snapshot_ctx->isp_snapshot_ctx->stream->cfg.width*snapshot_ctx->isp_snapshot_ctx->stream->cfg.height*3/2));
	else
...
int jpeg_snapshot_stream_change_resolution(int width,int height) //This is only for snapshot from streaming
{
	if(snapshot_ctx == NULL || snapshot_ctx->take_snapshot || snapshot_ctx->jpeg_ctx == NULL){
		printf("The snapshot is ongoing or snapshot_ctx is not initial\r\n");
		printf("snapshot_ctx = %x snapshot_ctx->take_snapshot = %x snapshot_ctx->jpeg_ctx =%x\r\n",snapshot_ctx,snapshot_ctx->take_snapshot,snapshot_ctx->jpeg_ctx);
                return -1;
	}
	if (snapshot_ctx->jpeg_ctx) {
		jpeg_release(snapshot_ctx->jpeg_ctx);
...
int snapshot_isp_config(int width,int height,int fps,int streamid,int format,int bayer_type)
{
	isp_init_cfg_t isp_init_cfg;
	isp_cfg_t cfg;
...
int snapshot_isp(unsigned char **raw_y)
{
	int ret = 0;
	if (snapshot_ctx == NULL) {
	...
	isp_stream_stop(snapshot_ctx->isp_snapshot_ctx->stream);
	if(snapshot_ctx->isp_snapshot_ctx->stream->cfg.format == ISP_FORMAT_YUV420_SEMIPLANAR)
		dcache_invalidate_by_addr((uint32_t *)snapshot_ctx->isp_snapshot_ctx->isp_buf.y_addr,snapshot_ctx->isp_snapshot_ctx->stream->cfg.height*snapshot_ctx->isp_snapshot_ctx->stream->cfg.width*3/2);
	else
...
{
	return snapshot_isp_config(width,height,fps,streamid,ISP_FORMAT_YUV420_SEMIPLANAR,0);
}

int yuv_snapshot_isp(unsigned char **raw_y)
{
	return snapshot_isp(raw_y);
}

int yuv_snapshot_isp_deinit()
{
	return snapshot_isp_deinit();
}


int yuv422_snapshot_isp_config(int width,int height,int fps,int streamid)
{
	return snapshot_isp_config(width,height,fps,streamid,ISP_FORMAT_YUV422_SEMIPLANAR,0);
}

int yuv422_snapshot_isp(unsigned char **raw_y)
{
	return snapshot_isp(raw_y);
}

int yuv422_snapshot_isp_deinit()
{
	return snapshot_isp_deinit();
}


int bayer_snapshot_isp_config(int width,int height,int fps,int streamid,int bayer_type)
{
	return snapshot_isp_config(width,height,fps,streamid,ISP_FORMAT_BAYER_PATTERN,bayer_type);
}

int bayer_snapshot_isp(unsigned char **raw_y)
{
	return snapshot_isp(raw_y);
}

int bayer_snapshot_isp_deinit()
{
	return snapshot_isp_deinit();
}
