		 ...
		 uint32_t* src_last_offset)
{
		...
		if (atom_type == FTYP_ATOM) {
			...
			*ftyp_buffer = (uint32_t)ftyp_atom;
			if (!ftyp_atom) {
				...
				printf ("could not allocate 0x%8X byte for ftyp atom\n",atom_size);
				f_close(&infile);
				return -1;
	...
	*moov_buffer = (uint32_t)moov_atom;
	...
	if (!moov_atom) {
		printf ("could not allocate 0x%8X byte for moov atom\n",atom_size);
		f_close(&infile);
		return -1;
	}
	...
	for (i = 4; i < moov_atom_size - 4; i++) {
		...
		if (atom_type == STCO_ATOM) {
			...
			offset_count = BE_32(&moov_atom[i + 8]);
			for (j = 0; j < offset_count; j++) {
				current_offset64 = BE_64(&moov_atom[i + 12 + j * 8]);
				current_offset64 += moov_atom_size;
				moov_atom[i + 12 + j * 8 + 0] = (current_offset64 >> 56) & 0xFF;
				moov_atom[i + 12 + j * 8 + 1] = (current_offset64 >> 48) & 0xFF;
				moov_atom[i + 12 + j * 8 + 2] = (current_offset64 >> 40) & 0xFF;
				moov_atom[i + 12 + j * 8 + 3] = (current_offset64 >> 32) & 0xFF;
				moov_atom[i + 12 + j * 8 + 4] = (current_offset64 >> 24) & 0xFF;
				moov_atom[i + 12 + j * 8 + 5] = (current_offset64 >> 16) & 0xFF;
				moov_atom[i + 12 + j * 8 + 6] = (current_offset64 >>  8) & 0xFF;
				moov_atom[i + 12 + j * 8 + 7] = (current_offset64 >>  0) & 0xFF;
			}
			i += atom_size - 4;
		}
	}
