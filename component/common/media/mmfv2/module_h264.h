...
#define CMD_H264_SET_BITRATE            MM_MODULE_CMD(0x16)
#define CMD_H264_SET_GOPLEN             MM_MODULE_CMD(0x17)
#define CMD_H264_SET_FPS                MM_MODULE_CMD(0x18) //For fps setting dynamic
#define CMD_H264_SET_RCPARM_UPDATE     MM_MODULE_CMD(0x19) //For rate control parameters
...
typedef struct h264_params_s{
	...
	uint32_t rst_rcerr;			// 1 : reset rc error when rate change
        ...
        uint32_t profile;
        uint32_t idrHeader;                     // Auto SPS&PPS Header
