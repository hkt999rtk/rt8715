...
#include "media_amebacam_broadcast.h"
#include "sockets.h"
...
void media_amebacam_broadcast_all(void);
...
static void amebacam_broadcast_thread_all(void *param)
{
	...
	if(setsockopt(bcastsocket, SOL_SOCKET, SO_BROADCAST, &broadcast, sizeof(broadcast)) < 0){
	...
	if(bind(bcastsocket, (struct sockaddr *) &bindAddr, sizeof(bindAddr)) < 0){
	...
	while(1) {
		...
		for(i=0;i<broadcast_retry_count;i++){
			if(sendto(bcastsocket, packet, strlen((char*)packet), 0, &to, sizeof(struct sockaddr))< 0)
...
static void amebacam_broadcast_thread(void *param)
{
	...
	while(1) {
		...
		if((packetLen = recvfrom(bcastsocket, packet, sizeof(packet), 0, &from, &fromLen)) >= 0) {
		...
		if(packetLen > 0) {
			...
			AmebaCam_device_name);	
#endif
			sendLen = sendto(bcastsocket, broadcast_to_app, strlen((const char*)broadcast_to_app), 0, &to, sizeof(struct sockaddr));
			if( sendLen < 0)
...
void media_amebacam_broadcast(void)
{
