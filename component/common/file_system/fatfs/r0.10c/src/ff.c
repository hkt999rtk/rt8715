			...
			if (res == FR_OK) {					/* Initialize the new directory table */
				...
				for (n = dj.fs->csize; n; n--) {	/* Write dot entries and clear following sectors */
					...
					mem_set(dir, 0, SS(dj.fs));
                                ...
                                printf("dj.fs->winsect %d\r\n",dj.fs->winsect);
#else
                                unsigned char *ptr = NULL;
                                dj.fs->winsect = dsc;
                                dj.fs->wflag =    1;
                                ptr = malloc(SS(dj.fs)*dj.fs->csize);
                                if(ptr == NULL){
                                        printf("Malloc memory fail\r\n");
                                        res = FR_DISK_ERR;
                                        goto mkdir_fail;
                                }
                                memset(ptr,0,SS(dj.fs)*dj.fs->csize);
                                memcpy(ptr,dir,SS(dj.fs));
                                if (disk_write(dj.fs->drv, ptr, dj.fs->winsect,dj.fs->csize)){
                                        res = FR_DISK_ERR;
                                        goto mkdir_fail;
                                }
                                mem_set(dir, 0, SS(dj.fs));
...
#if defined(CONFIG_PLATFORM_8195BHP)
#define SPEED_UP_FORMAT
#define SPEED_UP_BUF_SIZE (64*512)
#define SPEED_UP_BUF_BLOCK_SIZE (SPEED_UP_BUF_SIZE/SS(fs))
#endif
...
)
{
	...
	FATFS *fs;
	DSTATUS stat;
#if defined(CONFIG_PLATFORM_8195BHP) && defined(SPEED_UP_FORMAT)       
        unsigned char *buf = NULL;
        unsigned int temp_nfat = 0;
        //buf = malloc(SPEED_UP_BUF_SIZE);
        //if(buf == NULL){
          //printf("malloc fatfs buffer fail\r\n");
          //return FR_DISK_ERR;
        //}
        //memset(buf,0,SPEED_UP_BUF_SIZE);
#endif
	...
	for (i = 0; i < N_FATS; i++) {		/* Initialize each FAT copy */
		...
		//printf("%s %d n_fat = %d wsec = %d\r\n",__FUNCTION__,__LINE__,n_fat,wsect);
                //printf("sector size = %d\r\n",SS(fs));
#ifndef SPEED_UP_FORMAT
                for (n = 1; n < n_fat; n++) {		/* This loop may take a time on FAT32 volume due to many single sector writes */
			if (disk_write(pdrv, tbl, wsect++, 1))
				return FR_DISK_ERR;
		}
                //printf("After %s %d wsec = %d\r\n",__FUNCTION__,__LINE__,wsect);
#else
                buf = malloc(SPEED_UP_BUF_SIZE);
                if(buf == NULL){
                      printf("malloc fatfs buffer fail\r\n");
                ...
                memset(buf,0,SPEED_UP_BUF_SIZE);
                temp_nfat = n_fat - 1;
                for(n = 1; n <=(temp_nfat/SPEED_UP_BUF_BLOCK_SIZE); n++){
                      if(disk_write(pdrv, buf, wsect, SPEED_UP_BUF_BLOCK_SIZE)){
                            free(buf);
                            return FR_DISK_ERR;
                      }
                      wsect+=SPEED_UP_BUF_BLOCK_SIZE;
                }
                if(temp_nfat%SPEED_UP_BUF_BLOCK_SIZE){
                      if (disk_write(pdrv, buf, wsect, temp_nfat%SPEED_UP_BUF_BLOCK_SIZE)){
                              free(buf);
                              return FR_DISK_ERR;
                      }
                      wsect+=(temp_nfat%SPEED_UP_BUF_BLOCK_SIZE);
                }
                free(buf);
#endif
	}
	...
	do {
        ...
        //printf("after root wsect = %d\r\n",wsect);
          ...
          memset(buf,0,SPEED_UP_BUF_SIZE);
          for(n = 0; n < i/SPEED_UP_BUF_BLOCK_SIZE;n++){
                if (disk_write(pdrv, tbl, wsect, SPEED_UP_BUF_BLOCK_SIZE)){
                    if(buf)
                      free(buf);
                    return FR_DISK_ERR;
                }
          ...
          if(i%SPEED_UP_BUF_BLOCK_SIZE){
                if (disk_write(pdrv, tbl, wsect, i%SPEED_UP_BUF_BLOCK_SIZE)){
                        if(buf)
                          free(buf);
                            return FR_DISK_ERR;
                }
                wsect+=(i%SPEED_UP_BUF_BLOCK_SIZE);
          }
          //printf("after root wsect = %d\r\n",wsect);
          free(buf);
#endif
