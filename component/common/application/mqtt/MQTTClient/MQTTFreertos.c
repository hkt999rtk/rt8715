...
int FreeRTOS_write(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		...
		{
			getsockopt(n->my_socket, SOL_SOCKET, SO_ERROR, &so_error, (uint32_t *) &len);
			if (so_error != EAGAIN) {
				n->disconnect(n);
			}
			sentLen = rc;
...
int NetworkConnect(Network* n, char* addr, int port)
{
	...
	if ( n->use_ssl != 0 ) {
		...
		if (n->rootCA != NULL) {
			...
			if (mbedtls_x509_crt_parse( root_crt, (unsigned char const *) n->rootCA, strlen(n->rootCA)+1 ) != 0) {
				mqtt_printf(MQTT_DEBUG, "parse root_crt failed!");
				goto err;
			}
		...
		if (n->clientCA != NULL && n->private_key != NULL) {
			...
			if ( mbedtls_x509_crt_parse(client_crt, (unsigned char const *) n->clientCA, strlen(n->clientCA)+1) != 0 ) {
				mqtt_printf(MQTT_DEBUG, "parse client_crt failed!");
				goto err;
			}

			if ( mbedtls_pk_parse_key(client_rsa, (unsigned char const *) n->private_key, strlen(n->private_key)+1, NULL, 0) != 0 ) {
				mqtt_printf(MQTT_DEBUG, "parse client_rsa failed!");
				goto err;
			}
		}
