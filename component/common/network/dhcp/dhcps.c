...
volatile static uint8_t dhcps_num_of_available_ips;
#endif
...
struct dns_forward_state {
	struct udp_pcb *client_pcb;
	ip_addr_t client_addr;
	u16_t client_port;
};

static void dns_upstream_recv(void *arg, struct udp_pcb *recv_pcb,
	struct pbuf *recv_p, const ip_addr_t *recv_addr, u16_t recv_port)
{
	struct dns_forward_state *st = (struct dns_forward_state *)arg;
	(void)recv_addr; (void)recv_port;
	if (recv_p != NULL) {
		udp_sendto(st->client_pcb, recv_p, &st->client_addr, st->client_port);
		pbuf_free(recv_p);
	}
	udp_remove(recv_pcb);
	free(st);
...
struct pbuf *udp_packet_buffer, struct ip_addr *sender_addr, uint16_t sender_port)
{
	...
	struct dns_forward_state *state;
	volatile int ret = 0;

	(void)arg;

	ip4_addr_set_u32(ip_2_ip4(&upstream_dns), PP_HTONL(0x08080808));

	upstream_pcb = udp_new();
	if (upstream_pcb == NULL) {
		pbuf_free(udp_packet_buffer);
		return;
	}

	state = (struct dns_forward_state *)malloc(sizeof(struct dns_forward_state));
	if (state == NULL) {
		udp_remove(upstream_pcb);
		pbuf_free(udp_packet_buffer);
		return;
	}
	state->client_pcb = udp_pcb;
	ip_addr_set(&state->client_addr, sender_addr);
	state->client_port = sender_port;

	udp_recv(upstream_pcb, dns_upstream_recv, state);
	udp_bind(upstream_pcb, IP_ADDR_ANY, 0);
	ret = udp_sendto(upstream_pcb, udp_packet_buffer, &upstream_dns, 53);
...
#if LWIP_VERSION_MAJOR >= 2
	ip_addr_set_ip4_u32(&dhcps_network_id, (ip_addr_get_ip4_u32(netif_ip_addr4(pnetif))&(ip_addr_get_ip4_u32(netif_ip_netmask4(pnetif)))));
	ip_addr_set_ip4_u32(&dhcps_subnet_broadcast, (ip_addr_get_ip4_u32(&dhcps_network_id) | ~(ip_addr_get_ip4_u32(netif_ip_netmask4(pnetif)))));
#if 1
	ip_addr_set_ip4_u32(&dhcps_owned_first_ip, htonl(ntohl(ip_addr_get_ip4_u32(&dhcps_network_id)) + 1));
	ip_addr_set_ip4_u32(&dhcps_owned_last_ip, htonl(ntohl(ip_addr_get_ip4_u32(&dhcps_subnet_broadcast)) - 1));	
	dhcps_num_of_available_ips = (ntohl(ip_addr_get_ip4_u32(&dhcps_owned_last_ip)) - ntohl(ip_addr_get_ip4_u32(&dhcps_owned_first_ip))) + 1;
#endif
#else
...
#endif
#if LWIP_VERSION_MAJOR >= 2
	if(ip_addr_get_ip4_u32(&dhcps_addr_pool_start) == 0 && ip_addr_get_ip4_u32(&dhcps_addr_pool_end) == 0)
#else
