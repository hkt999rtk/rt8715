...
mm_context_t* array_h265_ctx   = NULL;
...
array_params_t h265usb_array_params = {
...
extern struct uvc_format_uncompressed uvc_format_h265;
...
void example_media_uvcd_init(void)
{
...
#endif
#if UVCD_H265
	uvc_format_h265.bFormatIndex = index_cnt++;
#endif
	...
	if(h264_ctx){
        ...
        array_h265_ctx = mm_module_open(&array_module);
        if(array_h265_ctx){
                mm_module_ctrl(array_h265_ctx, CMD_ARRAY_SET_PARAMS, (int)&h265usb_array_params);
                mm_module_ctrl(array_h265_ctx, CMD_ARRAY_SET_ARRAY, (int)&array);
                mm_module_ctrl(array_h265_ctx, MM_CMD_SET_QUEUE_LEN, 6);
                mm_module_ctrl(array_h265_ctx, MM_CMD_INIT_QUEUE_ITEMS, MMQI_FLAG_DYNAMIC);
                mm_module_ctrl(array_h265_ctx, CMD_ARRAY_APPLY, 0);
                mm_module_ctrl(array_h265_ctx, CMD_ARRAY_STREAMING, 1);	// streamming on
        }else{
	...
	if(siso_isp_uvcd){
        ...
        if(siso_array_uvcd){
                siso_ctrl(siso_array_uvcd, MMIC_CMD_ADD_INPUT, (uint32_t)array_h265_ctx, 0);
        ...
        while(1){
                ...
                if((uvc_format_local->format != uvc_format_ptr->format) || (uvc_format_local->width != uvc_format_ptr->width) || (uvc_format_local->height != uvc_format_ptr->height)){
                    ...
                    if((uvc_format_ptr->format == FORMAT_TYPE_YUY2) || (uvc_format_ptr->format == FORMAT_TYPE_NV12) || (uvc_format_ptr->format == FORMAT_TYPE_BAYER)){
                          ...
                          printf("siso_resume(siso_array_uvcd)\r\n");
                    }
                }
                ...
                if(uvc_format_local->state != uvc_format_ptr->state){
                        ...
                        if(uvc_format_ptr->state){
                                if((uvc_format_ptr->format == FORMAT_TYPE_YUY2) || (uvc_format_ptr->format == FORMAT_TYPE_NV12) || (uvc_format_ptr->format == FORMAT_TYPE_BAYER)){
                                          ...
                                          siso_resume(siso_array_uvcd);
                                          printf("power on siso_resume(siso_array_uvcd)\r\n");
#endif
#if UVCD_MJPG
                                    }else if(uvc_format_ptr->format == FORMAT_TYPE_MJPEG){
                                          mm_module_ctrl(isp_ctx, CMD_ISP_SET_PARAMS,(int)&isp_params);
