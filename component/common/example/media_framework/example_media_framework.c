...
#define MMF_THREAD_STACK_SIZE 2048
#else
#define MMF_THREAD_STACK_SIZE 512
#endif
...
void example_mmf2_signal_stream_main(void *param)
{
  //int ret;
	...
	//mmf2_example_a_init();
        ...
        // 1 Video (H264) 1 Audio (AAC+G711) dup -> 2 RTSP (V+A, V+A)
	...
	printf("\n\r webRTC Peer \n\r");
	// H264 and 2way audio (G711, PCMU)
	// ISP   -> H264 -> webRTC peer
	// AUDIO -> G711E  -> webRTC peer
	// RTP   -> G711D  -> AUDIO
	mmf2_example_webrtc_peer_init();
#endif
...
void example_media_framework(void)
{
	...
	if(xTaskCreate(example_mmf2_signal_stream_main, ((const char*)"mmf2_1"), MMF_THREAD_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL) != pdPASS) {
