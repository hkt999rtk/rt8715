...
extern SemaphoreHandle_t g_snapshot_sema;
extern unsigned int snapshot_width;
extern unsigned int snapshot_height;
extern int snapshot_format;
extern int snapshot_bayer_type;
...
void fATIS(void *arg)
{
	...
	if(!arg){
		printf("[ATIS] format fail.\n\r");
		goto exit;
	}
	argc = parse_param(arg, argv);

	snapshot_width = atoi(argv[1]);
	snapshot_height = atoi(argv[2]);
	snapshot_format = atoi(argv[3]);
	snapshot_bayer_type = atoi(argv[4]);
	
	if(g_snapshot_sema)
		xSemaphoreGive(g_snapshot_sema);
...
void fATIQ(void *arg)
{
	...
	else
	{
		...
		else if(dIndex==9)
		{
			...
			float starttime = 0;
			unsigned char startframe = 0;
			float endtime = 0;
			float endframe = 0;
			float diftime, difframe;
			int iFrameCntPre;
			int iOverflowCnt = 0;
			int iFPS=0;
...
void fATIR(void *arg)
{
	...
	argc = parse_param(arg, argv);
	mode = isp_str_to_value((unsigned char*)argv[1]);
