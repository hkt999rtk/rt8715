...
{
	memcpy(pwrq->ifrn_name, ifname, 5);
...
int wext_set_tos_value(const char *ifname, __u8 *tos_value)
{
	...
	//From ASD#1148, ASD#1175
	//BK = 32 or 70
	if((*tos_value == 32) || (*tos_value == 70)){
		*(para + cmd_len)   = 0x4f;
		*(para + cmd_len+1) = 0xa4;
		*(para + cmd_len+2) = 0;
		*(para + cmd_len+3) = 0;
	...
	//BE = 96 or 0
	else if((*tos_value == 90) || (*tos_value == 0)){
		*(para + cmd_len)   = 0x2b;
		*(para + cmd_len+1) = 0xa4;
		*(para + cmd_len+2) = 0;
		*(para + cmd_len+3) = 0;
	...
	//VO = 224 or 220
	else if((*tos_value == 224) || (*tos_value == 220)){
		*(para + cmd_len)   = 0x22;
		*(para + cmd_len+1) = 0x32;
		*(para + cmd_len+2) = 0x2f;
		*(para + cmd_len+3) = 0;
...
int wext_set_trp_tis(__u8 enable)
{
	...
	if(enable != RTW_TRP_TIS_DISABLE){
		...
		}else if(enable == RTW_TRP_TIS_FIX_PHY_ACK_HIGH_RATE){
			rtw_trp_tis_test_en = BIT3 | BIT0;
		}
