...
extern struct user_ip_config user_ip;

/*
 * The USB CDC ECM/NCM library owns the separate eth_netif object.  NET_IF_NUM
 * still includes CONFIG_ETHERNET, so do not also register that xnetif[] slot
 * as a WLAN/AP interface.
 */
#if (defined(CONFIG_ETHERNET) && CONFIG_ETHERNET) && \
	((defined(CONFIG_USBH_CDC_ECM) && CONFIG_USBH_CDC_ECM) || \
	 (defined(CONFIG_USBH_CDC_NCM) && CONFIG_USBH_CDC_NCM))
#define USB_CDC_OWNS_ETH_NETIF 1
#else
#define USB_CDC_OWNS_ETH_NETIF 0
#endif

void user_use_config_ip(void)
{
	user_ip.use_user_ip = 0;
}
void LwIP_Init(void)
{
	ip_addr_t ipaddr;
	...
	printf("NET_IF_NUM:%d\n\r",NET_IF_NUM);
	for(idx=0;idx<xnetif_count;idx++){
		if (idx == 0) {
			if (user_ip.use_user_ip) {
				uint8_t iptab[4];
				uint8_t nettab[4];
				uint8_t gwtab[4];
				iptab[3] = (uint8_t)(user_ip.addr >> 24);
				iptab[2] = (uint8_t)(user_ip.addr >> 16);
				iptab[1] = (uint8_t)(user_ip.addr >> 8);
				iptab[0] = (uint8_t)(user_ip.addr);
				printf("\n\r LwIP_Init Set Interface %d user IP : %d.%d.%d.%d\n", idx, iptab[3], iptab[2], iptab[1], iptab[0]);
				IP4_ADDR(ip_2_ip4(&ipaddr), iptab[0], iptab[1], iptab[2], iptab[3]);
				nettab[3] = (uint8_t)(user_ip.netmask >> 24);
				nettab[2] = (uint8_t)(user_ip.netmask >> 16);
				nettab[1] = (uint8_t)(user_ip.netmask >> 8);
				nettab[0] = (uint8_t)(user_ip.netmask);
				printf("\n\r LwIP_Init Set Interface %d user netmask : %d.%d.%d.%d\n", idx, nettab[3], nettab[2], nettab[1], nettab[0]);
				IP4_ADDR(ip_2_ip4(&netmask), nettab[0], nettab[1], nettab[2], nettab[3]);
				gwtab[3] = (uint8_t)(user_ip.gw >> 24);
				gwtab[2] = (uint8_t)(user_ip.gw >> 16);
				gwtab[1] = (uint8_t)(user_ip.gw >> 8);
				gwtab[0] = (uint8_t)(user_ip.gw);
				printf("\n\r LwIP_Init Set Interface %d user gw : %d.%d.%d.%d\n", idx, gwtab[3], gwtab[2], gwtab[1], gwtab[0]);			
				IP4_ADDR(ip_2_ip4(&gw), gwtab[0], gwtab[1], gwtab[2], gwtab[3]);
			}
			else
			{
		...
		printf("interface %d (%c%c) num=%d is initialized\n", idx, xnetif[idx].name[0], xnetif[idx].name[1], (int)netif_get_index(&xnetif[idx]));
	...
	eth_netif.name[0] = 'e';
	eth_netif.name[1] = 'n';
	static u8 mac[6] = {0x56,0x22,0x3E,0x03,0xD3,0x0F};	
	memcpy(eth_netif.hwaddr, mac, 6);

	netifapi_netif_add(&eth_netif, NULL, NULL, NULL, NULL, &ethernetif_mii_init, &tcpip_input);
	printf("interface %d (en) num=%d is initialized\n", NET_IF_NUM, (int)netif_get_index(&eth_netif));
#endif
...
#if CONFIG_FAST_DHCP
extern void addr_printf(u32_t addr);
				...
				if(is_the_same_ap){
					if( (offer_ip != 0 && offer_ip != 0xFFFFFFFF) || (dhcp != NULL) ){
						if(dhcp == NULL){
							...
							ip_addr_set_ip4_u32(&dhcp->server_ip_addr,server_ip);
...
#include "lwip/prot/dhcp.h"
			...
			else
			{
				...
				if (dhcp->tries > MAX_DHCP_TRIES)
				{
					...
					/* Static address used */
					if (user_ip.use_user_ip) {
						uint8_t iptab[4], nettab[4], gwtab[4];
						iptab[3] = (uint8_t)(user_ip.addr >> 24);
						iptab[2] = (uint8_t)(user_ip.addr >> 16);
						iptab[1] = (uint8_t)(user_ip.addr >> 8);
						iptab[0] = (uint8_t)(user_ip.addr);
...
uint32_t LwIP_GetXID(struct netif *pnetif)
{
	struct dhcp *dhcp = NULL;
#if LWIP_VERSION_MAJOR >= 2
	dhcp = ((struct dhcp*)netif_get_client_data(pnetif, LWIP_NETIF_CLIENT_DATA_INDEX_DHCP));
...
uint32_t LwIP_GetLEASETIME(struct netif *pnetif)
{
	struct dhcp *dhcp = NULL;
#if LWIP_VERSION_MAJOR >= 2
	dhcp = ((struct dhcp*)netif_get_client_data(pnetif, LWIP_NETIF_CLIENT_DATA_INDEX_DHCP));
...
uint32_t LwIP_GetRENEWTIME(struct netif *pnetif)
{
	struct dhcp *dhcp = NULL;
#if LWIP_VERSION_MAJOR >= 2
	dhcp = ((struct dhcp*)netif_get_client_data(pnetif, LWIP_NETIF_CLIENT_DATA_INDEX_DHCP));
...
uint8_t* LwIP_GetServerIP(struct netif *pnetif)
{
	struct dhcp *dhcp = NULL;
#if LWIP_VERSION_MAJOR >= 2
	dhcp = ((struct dhcp*)netif_get_client_data(pnetif, LWIP_NETIF_CLIENT_DATA_INDEX_DHCP));
...
void LwIP_SetIP(uint8_t idx, u32_t addr, u32_t netmask_addr, u32_t gw_addr)
{
	...
	ip_2_ip4(&gw)->addr = lwip_htonl(gw_addr);
