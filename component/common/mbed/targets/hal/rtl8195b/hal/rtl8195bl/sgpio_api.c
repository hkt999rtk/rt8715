...
void sgpio_init (sgpio_t *obj, PinName tx, PinName bi_dir_rx)
{
    ...
    sgpio_pin_sel_lp_t sgpio_pin_sel = Sgpio_PinSel_lp_Max;
    u16 pin_combin_tp;
    s32 pin_combin[2] = {(PA_3 << 8) | PA_2, (PA_7 << 8) | PA_8};

    pin_combin_tp = (u16)((bi_dir_rx << 8) | tx);

    if (pin_combin_tp == (u16)pin_combin[0]) {
        sgpio_pin_sel = Sgpio_PinSel_lp_0;
    } else if (pin_combin_tp == (u16)pin_combin[1]) {
        sgpio_pin_sel = Sgpio_PinSel_lp_1;
    }

    if (sgpio_pin_sel == Sgpio_PinSel_lp_Max) {
        DBG_SGPIO_ERR("sgpio_init, did not meet the pin group of sgpio \r\n");
    } else {
        DBG_SGPIO_INFO("sgpio_init pin, sgpio_pin_sel: %d\r\n", sgpio_pin_sel);
        hal_sgpio_init (psgpio_adapter, sgpio_pin_sel);
    }
