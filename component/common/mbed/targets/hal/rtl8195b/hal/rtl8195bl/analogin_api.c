...
static const PinMap PinMap_analogin[] = {
    ...
    {PA_7,  RTL_PIN_PERI(PID_ADC7, 7, 0), RTL_PIN_FUNC(PID_ADC0, 0)},
    {NC,    NC,     0}
};
...
uint8_t analogin_set_wake_rule (analogin_t * obj, uint16_t thld, uint8_t greater)
{
    ...
    hal_adc_calc_cali_val(adc_val, &adc_cali_paras_tmp);
...
/**
  * @brief  Analog input read internal resistor
  * @param  obj: adc object define in application software.
  * @retval : uint16_t: internal resistor
  */
uint16_t analogin_read_intl_r (void)
{
    uint16_t intl_r;
    hal_adc_read_cali_param(LP_ADC_INTER_RESISTANCE, (uint8_t*)&intl_r, 2);
