...
/**
 *  @brief   Read watchdog reset flag
 *
 *  @param   None
 *  @return  watchdog reset flag status
 *
 */
uint8_t check_wdt_rst_back(void) {
    return (hal_misc_wdt_read_lp_watchdog_flag());
}

/**
 *  @brief Set LP fastreboot and enable LP hw watchdog timer.
 *  @param[in] timeout_ms: the watch-dog timer timeout value, in ms.
 *             default action of timeout is to reset the whole system.
 *  @returns    void
 */
void enable_lp_watchdog(uint32_t timeout_ms) {
    hal_misc_wdt_enable_lp_watchdog(timeout_ms);
}
