...
void flash_init(flash_t *obj)
{
    ...
    if (pglob_spic_adaptor == NULL) {
        ret = spic_init(&hal_spic_adaptor, SpicQpiMode, obj->flash_pin_sel);
        if (ret != HAL_OK) {
...
void flash_erase_sector(flash_t *obj, uint32_t address)
{
    ...
    if ((phal_spic_adaptor->flash_type) == FLASH_TYPE_WINBOND) {
        status2_value = hal_flash_get_status(phal_spic_adaptor, cmd->rdsr2);
        if (status2_value & 0x40) {
            hal_flash_set_status(phal_spic_adaptor, cmd->wrsr2, status2_value & (~0x40));
        }
    }
...
It only supports winbond flash types in the AVL.
Please refer to the datatsheet of flash for more details of the content of status register 2.
...
{   
    ...
    if (phal_spic_adaptor->flash_type == FLASH_TYPE_ZBIT) {
        hal_flash_set_status(phal_spic_adaptor, (phal_spic_adaptor->cmd)->wrsr3, data | 0x10);
    } else {
...
/*
Function Description:
This function aims to reset the status register 1, please make sure the operation is appropriate.
...
{
    if (pglob_spic_adaptor == NULL) {
        spic_init(&hal_spic_adaptor, SpicQpiMode, flash_pin_sel);
    }
...
int flash_read_individual_lock_state(uint32_t address)
{
    ...
    } else if (FLASH_TYPE_WINBOND == flash_type) {
        if (state & 0x1) {
            DBG_SPIF_WARN("The section is protected\r\n");
        } else {
            DBG_SPIF_WARN("The section is not protected\r\n");
