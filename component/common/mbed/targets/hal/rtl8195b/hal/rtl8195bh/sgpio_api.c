...
void sgpio_init (sgpio_t *obj, PinName tx, PinName bi_dir_rx)
{
    ...
    sgpio_index_sel_t sgpio_index = Sgpio_Max_Id;
    sgpio_pin_sel_t sgpio_pin_sel = Sgpio_PinSel_Max;
    u8 i;
    u16 pin_combin_tp;
    s32 pin_combin[9] = {(PB_5 << 8) | PB_4, (PE_1 << 8) | PE_0, (PE_11 << 8) | PE_10,\
                         (PC_9 << 8) | PC_8, (PE_3 << 8) | PE_2, (PE_13 << 8) | PE_12,\
                         (PG_9 << 8) | PG_8, (PE_5 << 8) | PE_4, (PE_15 << 8) | PE_14};

    pin_combin_tp = (u16)((bi_dir_rx << 8) | tx);

    for (i = 0; i < 9; i++) {
        if (pin_combin_tp == (u16)pin_combin[i]) {

            if (i < 3) {
                sgpio_index = Sgpio0_Sel;
                sgpio_pin_sel = i;
            } else if ((i > 2) && (i < 6)) {
                sgpio_index = Sgpio1_Sel;
                sgpio_pin_sel = i - 3;
            } else if ((i > 5) && (i < 9)) {
                sgpio_index = Sgpio2_Sel;
                sgpio_pin_sel = i - 6;
            }
            DBG_SGPIO_INFO("sgpio_init, id:%d, sgpio_index:%d, sgpio_pin_sel: %d\r\n", i, sgpio_index, sgpio_pin_sel);
            hal_sgpio_init (psgpio_adapter, sgpio_index, sgpio_pin_sel);
    ...
    if (i == 9) {
        DBG_SGPIO_ERR("sgpio_init, did not meet the pin group of sgpio \r\n");
    }

    psgpio_adapter->dcache_memory_en = 1;
    psgpio_adapter->sgpio_sclk_ns = 12.5f; //ASIC
                                        ...
                                        u32 bit1_middle_change_time, u32 bit1_duration_time)
{
    ...
    if (bit0_duration_time < bit0_middle_change_time) {
        DBG_SGPIO_ERR("bit0_duration_time needs bigger than bit0_middle_change_time \r\n");
        error_temp = 1;
    } else {
    ...
    if (bit1_duration_time <  bit1_middle_change_time) {
        DBG_SGPIO_ERR("bit1_duration_time needs bigger than bit1_middle_change_time \r\n");
        error_temp = 1;
    } else {
